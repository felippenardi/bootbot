...
class BootBot extends EventEmitter {
  ...
  sendMessage(recipientId, message, options, pageId) {
    ...
    console.log({recipient, message, pageId});
    const req = () => (
      this.sendRequest({
        recipient,
        message,
  ...
  sendRequest(body, endpoint, method) {
    ...
    method = method || 'POST';
    const accessToken = this.accessToken[body.pageId];
  ...
  sendTypingIndicator(recipientId, milliseconds, pageId) {
    ...
    return new Promise((resolve, reject) => {
      return this.sendAction(recipientId, 'typing_on', {}, pageId).then(() => {
        setTimeout(() => this.sendAction(recipientId, 'typing_off', {}, pageId).then((json) => resolve(json)), timeout);
      });
    });
  }

  getUserProfile(userId, pageId) {
    const accessToken = this.accessToken[pageId];
    const url = `https://graph.facebook.com/v2.6/${userId}?fields=first_name,last_name,profile_pic,locale,timezone,gender&access_token=${accessToken}`;
    return fetch(url)
      .then(res => res.json())
  ...
  _handleMessageEvent(event) {
    ...
    this._hearMap.forEach(hear => {
      if (typeof hear.keyword === 'string' && hear.keyword.toLowerCase() === text.toLowerCase()) {
        const res = hear.callback.apply(this, [event, new Chat(this, pageId), {
